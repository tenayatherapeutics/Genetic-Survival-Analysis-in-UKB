...
pheno_filtered <- master_pheno %>%
  ...
  #          Digoxin ==0 & `Alpha-Adrenoceptor Blocking Drugs` ==0 &
  #          Diuretics == 0 ) %>%
  filter(Ezetimibe ==0 & `Centrally acting antihypertensive` ==0 & `Vasodilator antihypertensive drugs`==0 ) %>%
  left_join(target_df) %>%
...
tiff('/home/lzhang@TENAYA.local/ukbiobank/Paper_Figures/PCSK9_rs562556_IHD_CV_Outcome_LovePlot_WB.tiff', 
...
plotsurv(rsnum, gn, pheno_filtered, eval(pheno1), 'CV_outcome', 25, 0.0, adj=TRUE)
...
tiff('/home/lzhang@TENAYA.local/ukbiobank/Paper_Figures/PCSK9_rs562556_IHD_CV_Outcome_Unmatched_WB.tiff', width = 7, height = 6, units = 'in', res = 320)  #width=7, height=6)
print(unmatched)
dev.off()
